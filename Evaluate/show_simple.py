...
def show(datas):
    ...
    # 保存为 Excel 文件
    output_path = "../Result/Evaluation_Simple/output2.xlsx"
    df.to_excel(output_path, index=False, engine="openpyxl")
    print(f"JSON 数据已整理并导出为 Excel 文件：{output_path}")

def show0():
    with open(f'../Result/Evaluation_Simple/save_.json', 'r') as rf:
        datas = json.load(rf)
    rows = []
    for subject_name, s_datas in datas.items():
    ...
    # 保存为 Excel 文件
    output_path = "../Result/Evaluation_Simple/output1.xlsx"
    df.to_excel(output_path, index=False, engine="openpyxl")
    print(f"JSON 数据已整理并导出为 Excel 文件：{output_path}")
