...
def MBFL():
    ...
    for type_ in type_list:
        ...
        for subject_name in Subject_name:
            with open(f'../Result/Evaluation/{subject_name}_GBSR_FOR_MBFL_{type_}.json', 'r') as rf:
                datas = json.load(rf)
            print("GET JSON FILE FINISHED!", end='\n\n')
            # for formula_ in formula_list:
...
def main():
    ...
    for subject_name in Subject_name:
        ...
        for sbfl_ in SBFL_LIST:
            SBFL_JSON[sbfl_] = integrate(f'../Result/Evaluation/{subject_name}_{sbfl_}.json')
        return_json["SBFL"] = SBFL_JSON
        # 然后MBFL处理
        MBFL_JSON = {}
        for mbfl_ in MBFL_LIST:
            if mbfl_ == "MBFL":
                MBFL_JSON[mbfl_] = integrate(f'../Result/Evaluation/{subject_name}_{mbfl_}.json', 1)
            else:
                MBFL_JSON[mbfl_] = integrate(f'../Result/Evaluation/{subject_name}_{mbfl_}.json')
        ...
        #             datas = json.load(rf)
        #         print("GET JSON FILE FINISHED!", end='\n\n')
        #     in_data = integrate(f'../Result/Evaluation/{subject_name}_GBSR_FOR_MBFL_{type_}.json')
        #     for k, v in in_data.items():
        #         print(k, v)
        print(return_json)
        Save_json(return_json, f'../Result/Evaluation/{subject_name}_save_0.json')
        end_json[subject_name] = return_json
    Save_json(end_json, f'../Result/Evaluation/save_0.json')
    # toExcel(end_json)
    return
